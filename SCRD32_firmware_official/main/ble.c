...
#define BLE_TAG "BLE_WTLK"

#define PROFILE_NUM                 1
#define PROFILE_APP_IDX             0
#define ESP_APP_ID                  0x55
#define SAMPLE_DEVICE_NAME          "BLE_WTLK_"
...
bool gBleMsgRecived = false;
uint8_t gBleMsgRxLen = 0;
...
static void show_bonded_devices(void)
{
    ...
    EXAMPLE_DEBUG(BLE_TAG, "Bonded devices number : %d\n", dev_num);

    EXAMPLE_DEBUG(BLE_TAG, "Bonded devices list : %d\n", dev_num);
...
static void gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    switch (event) {
        ...
        case ESP_GAP_BLE_UPDATE_CONN_PARAMS_EVT:
            EXAMPLE_DEBUG(BLE_TAG, "update connection params status = %d, min_int = %d, max_int = %d,conn_int = %d,latency = %d, timeout = %d",
        ...
        case ESP_GAP_BLE_PASSKEY_REQ_EVT:                           /* passkey request event */
            EXAMPLE_DEBUG(BLE_TAG, "ESP_GAP_BLE_PASSKEY_REQ_EVT");
        ...
        case ESP_GAP_BLE_KEY_EVT:
            ...
            EXAMPLE_DEBUG(BLE_TAG, "key type = %s", esp_key_type_to_str(param->ble_security.ble_key.key_type));
            break;
        ...
        case ESP_GAP_BLE_REMOVE_BOND_DEV_COMPLETE_EVT: {
            EXAMPLE_DEBUG(BLE_TAG, "ESP_GAP_BLE_REMOVE_BOND_DEV_COMPLETE_EVT status = %d", param->remove_bond_dev_cmpl.status);
            #if DEBUG_ON
            esp_log_buffer_hex(BLE_TAG, (void *)param->remove_bond_dev_cmpl.bd_addr, sizeof(esp_bd_addr_t));
            #endif
            EXAMPLE_DEBUG(BLE_TAG, "------------------------------------");
            break;
        }
        default:
            break;
...
{
    EXAMPLE_DEBUG(BLE_TAG, "prepare write, handle = %d, value len = %d", param->write.handle, param->write.len);
    ...
    if (param->exec_write.exec_write_flag == ESP_GATT_PREP_WRITE_EXEC && prepare_write_env->prepare_buf){
        if(prepare_write_env->prepare_len == 256) {
            ...
            if(long_write_success) {
                ESP_LOGI(BLE_TAG, "(4) ***** long write success ***** \n");
            }
        }
    }else{
...
{
    esp_ble_gatts_send_indicate(gatts_if, conn_id, gatt_db_handle_table[IDX_CHAR_VAL_C],
                                len, data, false);
}
...
static void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    switch (event) {
        ...
        case ESP_GATTS_READ_EVT:
            ...
            //ESP_LOGE(BLE_TAG, "ESP_GATTS_READ_EVT, handle=0x%d, offset=%d", param->read.handle, param->read.offset);
            if(gatt_db_handle_table[IDX_CHAR_VAL_A] == param->read.handle) {
                ESP_LOGE(BLE_TAG, "(2) ***** read char1 ***** \n");
            }
            if(gatt_db_handle_table[IDX_CHAR_VAL_B] == param->read.handle) {
                ESP_LOGE(BLE_TAG, "(5) ***** read char2 ***** \n");
            }
       	    break;
            ...
            if (!param->write.is_prep){
                ...
                if (gatt_db_handle_table[IDX_CHAR_CFG_C_2] == param->write.handle && param->write.len == 2){
                    ...
                    if (descr_value == 0x0001){
                        ...
                        ESP_LOGI(BLE_TAG, "(6) ***** send notify Connected ***** \n");
                ...
                if(gatt_db_handle_table[IDX_CHAR_VAL_A] == param->write.handle/* && param->write.len == 15*/) {
                    // uint8_t write_data[2] = {0x88, 0x99};
                    // if(memcmp(write_data, param->write.value, param->write.len) == 0) {
                    //     ESP_LOGI(BLE_TAG, "(3)***** short write success ***** \n");
                    // }
                    ESP_LOGI(BLE_TAG, "RxStr:%s", param->write.value);
                }

                if(gatt_db_handle_table[IDX_CHAR_VAL_B] == param->write.handle) {
                    memset(BleRxBuf, 0, 256);
                    memcpy(BleRxBuf, param->write.value, param->write.len);
                    gBleMsgRecived = true;
                    gBleMsgRxLen = param->write.len;
                    ESP_LOGI(BLE_TAG, "param->write.need_rsp = %d, BleRxString: %s", param->write.need_rsp, param->write.value);

                    gGatts_if = gatts_if;
                    gBleConnID = param->write.conn_id;
                }
                /* send response when param->write.need_rsp is true*/
                if (param->write.need_rsp)
                {
                ...
                ESP_LOGI(BLE_TAG, "handle prepare write");
                example_prepare_write_event_env(gatts_if, &prepare_write_env, param);
            }
      	    break;
        case ESP_GATTS_EXEC_WRITE_EVT:
            // the length of gattc prepare write data must be less than GATTS_EXAMPLE_CHAR_VAL_LEN_MAX.
            ESP_LOGI(BLE_TAG, "ESP_GATTS_EXEC_WRITE_EVT, Length=%d",  prepare_write_env.prepare_len);
            example_exec_write_event_env(&prepare_write_env, param);
            break;
        case ESP_GATTS_MTU_EVT:
            EXAMPLE_DEBUG(BLE_TAG, "ESP_GATTS_MTU_EVT, MTU %d", param->mtu.mtu);
            break;
        case ESP_GATTS_CONF_EVT:
            EXAMPLE_DEBUG(BLE_TAG, "ESP_GATTS_CONF_EVT, status = %d", param->conf.status);
            break;
